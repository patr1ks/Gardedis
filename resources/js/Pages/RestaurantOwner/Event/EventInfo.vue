    ...
    try {
        await router.post('events/store', formData, {
            ...
            onSuccess: (page) => {
                ...
                resetFormData();
                router.visit('/restaurant-owner/events');
            },
        });
    } catch (err) {
        console.error(err);
...
const deleteImage = async (eimage, index) => {
    try {
        await router.delete('/restaurant-owner/events/image/'+eimage.id, {
            onSuccess: (page) => {
